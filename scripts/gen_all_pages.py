#!/usr/bin/env python3
"""Generate all remaining Wingman.vip page files."""
import os
...
const SOUL_FORGE_TRAITS = [
  ...
  { id: "gratitude", label: "Gratitude", desc: "Appreciation for life's gifts", category: "Heart" },
...
const AVATARS = [
  { id: "cosmic", label: "Cosmic", emoji: "🌌", color: "from-violet-600 to-purple-800" },
  { id: "aurora", label: "Aurora", emoji: "🌠", color: "from-cyan-500 to-blue-600" },
  { id: "ember", label: "Ember", emoji: "🔥", color: "from-orange-500 to-red-600" },
  { id: "forest", label: "Forest", emoji: "🌿", color: "from-emerald-500 to-green-700" },
  { id: "ocean", label: "Ocean", emoji: "🌊", color: "from-blue-500 to-cyan-600" },
  { id: "solar", label: "Solar", emoji: "☀️", color: "from-yellow-400 to-orange-500" },
];
...
export default function Onboarding() {
  ...
  const [avatar, setAvatar] = useState("cosmic");
  ...
  const createWingman = trpc.wingman.create.useMutation();
  const updateTraits = trpc.soulForge.updateTraits.useMutation();
  const updateInterests = trpc.soulForge.updateInterests.useMutation();
  ...
  const handleActivate = async () => {
    ...
    setActivating(true);
    try {
      await createWingman.mutateAsync({
      ...
      await updateTraits.mutateAsync({ traits });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="pt-28 pb-20 px-4 max-w-3xl mx-auto relative z-10">
        ...
        {step === 1 && (
          <div className="text-center animate-fade-up">
            ...
            <h1 className="font-display text-4xl sm:text-5xl font-bold mb-4">
              Welcome to <span className="gradient-text">Soul Forge</span>
            </h1>
            ...
            <div className="grid sm:grid-cols-3 gap-4 mb-10">
              {[
                { icon: Brain, title: "34 Traits", desc: "Deep personality mapping" },
                { icon: Heart, title: "Your Values", desc: "What matters most to you" },
                { icon: Zap, title: "AI Activation", desc: "Your Wingman comes alive" },
        ...
        {/* STEP 2: Soul Forge Traits */}
        {step === 2 && (
          <div className="animate-fade-up">
            <div className="text-center mb-8">
              <Badge className="mb-3 bg-primary/10 text-primary border-primary/20">Soul Forge Assessment</Badge>
        ...
        {step === 3 && (
          <div className="animate-fade-up">
            ...
            {interests.length > 0 && (
              <p className="text-center text-sm text-primary mt-6">{interests.length} selected</p>
            )}
          </div>
        )}
          ...
          <div className="animate-fade-up">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
              {AVATARS.map(av => (
                ...
                >
                  <div className={"w-16 h-16 rounded-2xl bg-gradient-to-br " + av.color + " flex items-center justify-center mx-auto mb-3 text-2xl shadow-lg"}>
                    {av.emoji}
                  </div>
            ...
            <div className="glass-card p-6">
              ...
              <input
                type="text"
                value={wingmanName}
                onChange={e => setWingmanName(e.target.value)}
        ...
        {step === 7 && (
          <div className="text-center animate-fade-up">
            ...
            <h2 className="font-display text-4xl font-bold mb-4">
              {activating ? "Activating..." : "Ready to Activate?"}
            </h2>
            <p className="text-muted-foreground text-lg max-w-xl mx-auto mb-8">
              {activating
                ? "Your Wingman is coming to life. Calibrating personality matrix and initializing discovery protocols..."
                : `Your Wingman "${wingmanName || "unnamed"}" is ready to be activated. Once live, it will begin discovering compatible connections on your behalf.`}
            </p>
            {!activating && (
...
import { Progress } from "@/components/ui/progress";
import {
  Zap, Brain, Heart, Activity, Users, Shield, Tv,
  ArrowRight, Bell, Compass, Star, TrendingUp, Globe
} from "lucide-react";
import { toast } from "sonner";
...
export default function Dashboard() {
  ...
  const { data: connections } = trpc.connections.getMyConnections.useQuery({ limit: 5 }, { enabled: isAuthenticated });
  const { data: stories } = trpc.wingman.getStories.useQuery({ limit: 3 }, { enabled: isAuthenticated });
  const { data: dreamBoard } = trpc.discovery.getDreamBoard.useQuery({ limit: 4 }, { enabled: isAuthenticated });
  const { data: notifications } = trpc.notifications.getAll.useQuery({ limit: 5 }, { enabled: isAuthenticated });

  const activateWingman = trpc.wingman.activate.useMutation({
      ...
      wsRef.current.onmessage = (e) => {
        try {
          ...
          if (data.type === "activity") {
            setTickerItems(prev => [data.message, ...prev.slice(0, 19)]);
          }
        } catch {}
      };
    } catch {}
  ...
  useEffect(() => {
    if (activityFeed) {
      setTickerItems(activityFeed.map(a => a.description));
    }
  }, [activityFeed]);
            ...
            <div className="glass-card p-6 holographic">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                {[
                  ...
                  { label: "Trust Score", value: (wingman.trustScore || 85) + "%", icon: Shield, color: "text-emerald-400" },
              ...
              <div className="space-y-3">
                ...
                <CompatibilityMeter score={94} label="Overall Match Quality" color="text-primary" />
            ...
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-display font-semibold">Dream Board</h2>
                </div>
                <Link href="/dream-board">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {(dreamBoard || []).slice(0, 4).map((entry: any, i: number) => (
                  <div key={entry.id || i} className="glass rounded-xl p-3 text-center border border-border hover:border-primary/40 transition-all cursor-pointer">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-600 to-cyan-500 flex items-center justify-center mx-auto mb-2 text-sm font-bold text-white">
                      {(entry.targetWingmanName || "W")[0]}
                    </div>
                    <p className="text-xs font-medium truncate">{entry.targetWingmanName || "Wingman"}</p>
                ...
                {(!dreamBoard || dreamBoard.length === 0) && (
                  <div className="col-span-4 text-center py-6 text-muted-foreground text-sm">
                    Your Wingman is searching for matches...
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-3">
                ...
                {(!stories || stories.length === 0) && (
                  <div className="text-center py-4 text-muted-foreground text-sm">
                    Stories will appear as your Wingman makes connections...
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="glass-card p-5">
              ...
              <div className="space-y-3">
                {(connections || []).slice(0, 5).map((conn: any, i: number) => (
                  <div key={conn.id || i} className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-600 to-cyan-500 flex items-center justify-center text-xs font-bold text-white">
                      {(conn.connectedWingmanName || "W")[0]}
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-medium truncate">{conn.connectedWingmanName || "Wingman"}</p>
                ...
                {(!connections || connections.length === 0) && (
                  <p className="text-xs text-muted-foreground text-center py-2">No connections yet</p>
                )}
              </div>
            </div>
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Compass, Users, MapPin, Filter, Heart, Brain, Zap, ArrowRight, Globe } from "lucide-react";
import { toast } from "sonner";
...
export default function Discovery() {
  ...
  const { data: spaces } = trpc.discovery.getVirtualSpaces.useQuery({ mode: selectedMode === "all" ? undefined : selectedMode });
  const { data: wingman } = trpc.wingman.getMyWingman.useQuery(undefined, { enabled: isAuthenticated });
  const { data: dreamBoard } = trpc.discovery.getDreamBoard.useQuery({ limit: 12 }, { enabled: isAuthenticated });

  const enterSpace = trpc.discovery.enterSpace.useMutation({
  ...
  const requestIntro = trpc.discovery.requestIntroduction.useMutation({
    onSuccess: () => toast.success("Introduction request sent! Your Wingman will handle it."),
    onError: () => toast.error("Could not send introduction request"),
  });
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="font-display text-3xl font-bold mb-1">
              <span className="gradient-text">Discovery</span> Engine
            </h1>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Globe className="w-4 h-4 text-cyan-400" /> Virtual Spaces
            </h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {(spaces || []).map((space: any) => (
                <div
                  key={space.id}
                  className={"glass-card p-5 cursor-pointer transition-all border-2 " + (selectedSpace === space.id ? "border-primary bg-primary/5" : "border-transparent hover:border-primary/30")}
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      <span>{space.activeUsers || 0} active</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" className="btn-aurora text-xs px-3 py-1 h-auto"
                      onClick={e => { e.stopPropagation(); if (wingman) enterSpace.mutate({ spaceId: space.id }); else toast.error("Activate your Wingman first"); }}>
          ...
          <div>
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Heart className="w-4 h-4 text-rose-400" /> Potential Connections
            </h2>
            <div className="space-y-3">
              {(dreamBoard || []).slice(0, 6).map((entry: any, i: number) => (
                <div key={entry.id || i} className="glass-card p-4 hover:border-primary/30 transition-all border border-transparent">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold">{entry.targetWingmanName || "Wingman"}</p>
                  ...
                  <Button size="sm" variant="outline" className="w-full text-xs glass border-border hover:border-primary/40"
                    onClick={() => requestIntro.mutate({ targetWingmanId: entry.targetWingmanId, message: "Your Wingman would like to introduce itself." })}>
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Shield, Lock, Unlock, ArrowUp, Users, Star } from "lucide-react";
import { toast } from "sonner";

const TRUST_LEVELS = [
  { level: 1, name: "Public", desc: "Open discovery — minimal info shared", color: "text-muted-foreground", bg: "bg-muted/20", border: "border-muted/30" },
  { level: 2, name: "Acquaintance", desc: "Basic profile visible to new connections", color: "text-blue-400", bg: "bg-blue-400/10", border: "border-blue-400/30" },
  { level: 3, name: "Connection", desc: "Shared interests and compatibility data", color: "text-cyan-400", bg: "bg-cyan-400/10", border: "border-cyan-400/30" },
  { level: 4, name: "Trusted", desc: "Deep personality insights shared", color: "text-emerald-400", bg: "bg-emerald-400/10", border: "border-emerald-400/30" },
  { level: 5, name: "Inner Circle", desc: "Full access — reserved for close connections", color: "text-primary", bg: "bg-primary/10", border: "border-primary/30" },
];

export default function TrustLadder() {
  const { isAuthenticated } = useAuth();
  const { data: connections } = trpc.connections.getMyConnections.useQuery({ limit: 50 }, { enabled: isAuthenticated });
  const updateTrust = trpc.connections.updateTrustLevel.useMutation({
    onSuccess: () => toast.success("Trust level updated"),
    onError: () => toast.error("Failed to update trust level"),
  });

  const connectionsByLevel = TRUST_LEVELS.map(tl => ({
    ...tl,
    connections: (connections || []).filter((c: any) => c.trustLevel === tl.level),
  }));
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="font-display text-3xl font-bold mb-1">
            <span className="gradient-text">Trust</span> Ladder
          </h1>
        ...
        <div className="space-y-6">
          {connectionsByLevel.map(({ level, name, desc, color, bg, border, connections: levelConns }) => (
            <div key={level} className={"glass-card p-6 border " + border}>
              ...
              {levelConns.length > 0 ? (
                <div className="grid sm:grid-cols-2 gap-3">
                  {levelConns.map((conn: any) => (
                    <div key={conn.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/10 border border-border">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium truncate">{conn.connectedWingmanName}</p>
                        <p className="text-xs text-muted-foreground">{conn.connectionType} • {conn.compatibilityScore}% match</p>
                      </div>
                      <div className="flex gap-1 ml-auto">
                        {level > 1 && (
                          <button onClick={() => updateTrust.mutate({ connectionId: conn.id, trustLevel: level - 1 })}
                ...
                <div className="text-center py-4 text-muted-foreground text-sm">
                  No connections at this trust level yet
                </div>
              )}
            </div>
          ))}
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Users, MessageCircle, Zap, Globe, Heart, Star } from "lucide-react";
import { toast } from "sonner";
...
export default function SocialLounge() {
  ...
  const { data: spaces } = trpc.discovery.getVirtualSpaces.useQuery({ mode: activeMode });
  const { data: connections } = trpc.connections.getMyConnections.useQuery({ limit: 20 }, { enabled: isAuthenticated });
  const enterSpace = trpc.discovery.enterSpace.useMutation({
  ...
  return (
    <AppLayout>
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="font-display text-3xl font-bold mb-1">
            <span className="gradient-text">Social</span> Lounge
          </h1>
          <p className="text-muted-foreground text-sm">Your Wingman's social headquarters</p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Globe className="w-4 h-4 text-cyan-400" /> Active Spaces
            </h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {(spaces || []).map((space: any) => (
                <div key={space.id} className="glass-card p-5 border border-transparent hover:border-primary/30 transition-all">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Users className="w-3 h-3" />{space.activeUsers || 0}
                    </div>
                  </div>
                  ...
                  <Button size="sm" className="btn-aurora w-full text-xs"
                    onClick={() => enterSpace.mutate({ spaceId: space.id })}>
                    Enter Space
                  </Button>
                </div>
              ))}
          ...
          <div>
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Heart className="w-4 h-4 text-rose-400" /> Your Connections
            </h2>
            <div className="space-y-3">
              {(connections || []).filter((c: any) => c.connectionType === activeMode).map((conn: any) => (
                <div key={conn.id} className="glass-card p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium">{conn.connectedWingmanName}</p>
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Plane, MapPin, Users, Calendar, Zap } from "lucide-react";
import { toast } from "sonner";
...
export default function Travel() {
  ...
  const { data: travelEvents } = trpc.travel.getMyTravelEvents.useQuery(undefined, { enabled: isAuthenticated });
  const { data: nearbyConnections } = trpc.travel.getNearbyConnections.useQuery(
    { city, radius: 50 },
    { enabled: isAuthenticated && city.length > 2 }
  );

  const addTravel = trpc.travel.addTravelEvent.useMutation({
    onSuccess: () => { toast.success("Travel event added! Your Wingman will find connections in " + city); setCity(""); },
    onError: () => toast.error("Failed to add travel event"),
  });
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="font-display text-3xl font-bold mb-1">
            <span className="gradient-text">Travel</span> Intelligence
          </h1>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="glass-card p-6">
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Plane className="w-4 h-4 text-blue-400" /> Add Travel Plan
            </h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Destination City</label>
                <input
                  value={city}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">Arrival Date</label>
                  <input
                    type="date"
                    value={arrivalDate}
              ...
              <Button className="btn-aurora w-full"
                onClick={() => {
                  if (!city || !arrivalDate || !departureDate) { toast.error("Fill in all fields"); return; }
                  addTravel.mutate({ city, arrivalDate: new Date(arrivalDate), departureDate: new Date(departureDate) });
                }}>
          ...
          <div className="glass-card p-6">
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Users className="w-4 h-4 text-emerald-400" /> Connections in {city || "Your City"}
            </h2>
            {city.length > 2 ? (
              <div className="space-y-3">
                  ...
                  <div key={conn.id || i} className="flex items-center gap-3 p-3 rounded-xl bg-muted/20">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        <MapPin className="w-3 h-3" /> {conn.city}
                      </p>
                    </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground">Enter a city to find nearby connections</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-8">
          <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
            <Calendar className="w-4 h-4 text-amber-400" /> Your Travel Events
          </h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(travelEvents || []).map((event: any) => (
              <div key={event.id} className="glass-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  <Plane className="w-4 h-4 text-blue-400" />
                  <h3 className="font-semibold">{event.city}</h3>
                  <Badge className={"ml-auto text-[10px] " + (event.status === "active" ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-muted/30 text-muted-foreground border-border")}>
                    {event.status}
                  </Badge>
                </div>
                <p className="text-xs text-muted-foreground">
                  {new Date(event.arrivalDate).toLocaleDateString()} — {new Date(event.departureDate).toLocaleDateString()}
                </p>
                <p className="text-xs text-primary mt-2">{event.connectionsFound || 0} connections found</p>
              </div>
            ))}
            {(!travelEvents || travelEvents.length === 0) && (
              <div className="col-span-3 glass-card p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">No travel events yet. Add your first trip above.</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="font-display text-3xl font-bold mb-1">
            <span className="gradient-text">Conference</span> Matching
          </h1>
          ...
          {(conferences || []).map((conf: any) => (
            <div key={conf.id} className="glass-card p-6 border border-transparent hover:border-primary/30 transition-all">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                  <Users className="w-3 h-3" />{conf.attendeeCount || 0}
                </div>
              </div>
              ...
              <div className="space-y-1.5 mb-4">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>{conf.startDate ? new Date(conf.startDate).toLocaleDateString() : "TBD"}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <Button size="sm" className="btn-aurora w-full text-xs"
                onClick={() => {
                  if (!wingman) { toast.error("Activate your Wingman first"); return; }
                  register.mutate({ conferenceId: conf.id, connectionGoals: { networking: true, collaboration: true } });
                }}>
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Bell, Check, Trash2, Heart, Users, Shield, Zap, Plane } from "lucide-react";
import { toast } from "sonner";

const NOTIF_ICONS: Record<string, React.ElementType> = {
  connection: Heart, introduction: Users, trust: Shield, wingman: Zap, travel: Plane,
};
    ...
    <AppLayout>
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          {unreadCount > 0 && (
            <Button variant="outline" size="sm" className="glass border-border" onClick={() => markAllRead.mutate()}>
        ...
        <div className="space-y-3">
          {(notifications || []).map((notif: any) => {
            ...
            return (
              <div
                key={notif.id}
                className={"glass-card p-4 flex items-start gap-4 border transition-all " + (!notif.isRead ? "border-primary/20 bg-primary/5" : "border-transparent")}
              >
                <div className={"w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 " + (!notif.isRead ? "bg-primary/20" : "bg-muted/20")}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {!notif.isRead && <Badge className="text-[10px] bg-primary/10 text-primary border-primary/20 flex-shrink-0">New</Badge>}
                  </div>
                ...
                {!notif.isRead && (
                  <button onClick={() => markRead.mutate({ notificationId: notif.id })}
                    className="p-1.5 rounded-lg hover:bg-muted/20 transition-colors flex-shrink-0">
          ...
          {(!notifications || notifications.length === 0) && (
            <div className="glass-card p-16 text-center">
              ...
              <h3 className="font-display font-semibold mb-2">All Clear</h3>
              <p className="text-muted-foreground text-sm">No notifications yet. Your Wingman will alert you when something happens.</p>
            </div>
          )}
        </div>
      </div>
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { User, Brain, Shield, Star, Edit2, Save, Zap } from "lucide-react";
import { toast } from "sonner";
...
export default function Profile() {
  ...
  const { data: wingman } = trpc.wingman.getMyWingman.useQuery(undefined, { enabled: isAuthenticated });
  const { data: soulForge } = trpc.soulForge.getMyAssessment.useQuery(undefined, { enabled: isAuthenticated });
  const { data: connections } = trpc.connections.getMyConnections.useQuery({ limit: 100 }, { enabled: isAuthenticated });
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="glass-card p-6 text-center">
            ...
            <div className="grid grid-cols-2 gap-3 text-center">
              <div className="p-2 rounded-xl bg-muted/20">
                <p className="font-bold text-primary">{(connections || []).length}</p>
                <p className="text-[10px] text-muted-foreground">Connections</p>
              </div>
              <div className="p-2 rounded-xl bg-muted/20">
                <p className="font-bold text-cyan-400">{wingman?.totalIntroductions || 0}</p>
          ...
          <div className="glass-card p-6">
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Brain className="w-4 h-4 text-primary" /> My Wingman
            </h2>
            {wingman ? (
              <div>
                <div className="flex items-center gap-3 mb-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-muted-foreground">Trust Score</span><span className="text-emerald-400">{wingman.trustScore || 85}%</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Social Modes</span><span>{(wingman.socialModes || []).join(", ")}</span></div>
                </div>
              </div>
            ) : (
              <div className="text-center py-6">
                <Brain className="w-10 h-10 text-muted-foreground mx-auto mb-3" />
                <p className="text-sm text-muted-foreground mb-3">No Wingman yet</p>
                <Button size="sm" className="btn-aurora">Forge Wingman</Button>
              </div>
            )}
          </div>
          ...
          <div className="glass-card p-6">
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Zap className="w-4 h-4 text-amber-400" /> Soul Forge
            </h2>
            {soulForge ? (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground mb-3">Top personality traits</p>
                {Object.entries(soulForge.traits || {}).slice(0, 8).map(([trait, score]: [string, any]) => (
                  <div key={trait}>
              ...
              <div className="text-center py-6">
                ...
                <p className="text-sm text-muted-foreground">Complete Soul Forge assessment</p>
              </div>
            )}
          </div>
        </div>
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Tv, Play, Zap, Heart, Star, RefreshCw } from "lucide-react";
import { toast } from "sonner";
...
export default function WingmanTV() {
  ...
  const { data: stories, refetch } = trpc.wingman.getStories.useQuery({ limit: 20 }, { enabled: isAuthenticated });
    ...
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="font-display text-3xl font-bold mb-1">
              <span className="gradient-text">Wingman</span> TV
            </h1>
          ...
          <div className="glass-card p-8 mb-8 holographic">
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setPlayingId(playingId === stories[0].id ? null : stories[0].id)}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {(!stories || stories.length === 0) && (
            <div className="col-span-3 glass-card p-16 text-center">
              ...
              <Button className="btn-aurora" onClick={() => generateStory.mutate()}>
                <Zap className="w-4 h-4 mr-2" /> Generate First Episode
              </Button>
            </div>
          )}
        </div>
...
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Heart, Brain, Star, Filter, Zap, Users } from "lucide-react";
import { toast } from "sonner";
...
export default function DreamBoard() {
  ...
  const { data: dreamBoard } = trpc.discovery.getDreamBoard.useQuery({ limit: 50 }, { enabled: isAuthenticated });
  const requestIntro = trpc.discovery.requestIntroduction.useMutation({
    onSuccess: () => toast.success("Introduction requested! Your Wingman will handle it."),
    onError: () => toast.error("Failed to request introduction"),
  });
    ...
    <AppLayout>
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="font-display text-3xl font-bold mb-1">
              <span className="gradient-text">Dream</span> Board
            </h1>
          ...
          {filtered.map((entry: any, i: number) => (
            <div key={entry.id || i} className="glass-card p-5 border border-transparent hover:border-primary/30 transition-all group">
              ...
              <div className="text-center mb-3">
                <h3 className="font-display font-semibold">{entry.targetWingmanName || "Wingman"}</h3>
              ...
              {entry.sharedTraits && entry.sharedTraits.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                  {entry.sharedTraits.slice(0, 3).map((trait: string) => (
              ...
              <Button size="sm" className="w-full text-xs btn-aurora opacity-0 group-hover:opacity-100 transition-opacity"
                onClick={() => requestIntro.mutate({ targetWingmanId: entry.targetWingmanId, message: "Your Wingman would like to connect." })}>
...
import AppLayout from "@/components/AppLayout";
import { Badge } from "@/components/ui/badge";
import { Users, Brain, Activity, TrendingUp, Shield, Zap, BarChart2, Globe } from "lucide-react";
import { useLocation } from "wouter";
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          {[
            ...
            <div key={label} className="glass-card p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <Badge className="text-[10px] bg-muted/30 text-muted-foreground border-border">30d</Badge>
              </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              {(recentUsers || []).map((u: any) => (
                <div key={u.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-muted/10">
                  ...
                  <Badge className={"ml-auto text-[10px] " + (u.role === "admin" ? "bg-primary/10 text-primary border-primary/20" : "bg-muted/30 text-muted-foreground border-border")}>
                    {u.role}
                  </Badge>
                </div>
              ))}
          ...
          <div className="glass-card p-6">
            <h2 className="font-display font-semibold mb-4 flex items-center gap-2">
              <Brain className="w-4 h-4 text-cyan-400" /> Active Wingmen
            </h2>
            <div className="space-y-3">
              {(activeWingmen || []).map((w: any) => (
                <div key={w.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-muted/10">
                  <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-violet-600 to-cyan-500 flex items-center justify-center text-xs font-bold text-white">
                    {(w.wingmanName || "W")[0]}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{w.wingmanName}</p>
                    <p className="text-xs text-muted-foreground">{w.personalityArchetype} • {w.totalConnections} connections</p>
                  </div>
              ...
              {(!activeWingmen || activeWingmen.length === 0) && (
                <p className="text-sm text-muted-foreground text-center py-4">No active Wingmen</p>
              )}
            </div>
          </div>
        </div>
...
print("Files written:")
for f in ["Home.tsx", "Onboarding.tsx", "Dashboard.tsx", "Discovery.tsx",
          "TrustLadder.tsx", "SocialLounge.tsx", "Travel.tsx", "Events.tsx",
          "Notifications.tsx", "Profile.tsx", "WingmanTV.tsx", "DreamBoard.tsx", "Admin.tsx"]:
